...
class Query_list:
    ...
    def create_query_set(self, query_type):
        ...
        if query_type is Query_type.CAREER_DEP_ARRV_DELAY_IN_AIRPORT:
            self.get_USFlightDelay_list()
        elif query_type is Query_type.US_ACCIDENT_OCURRENCE:
            self.get_USAccidents_list()
        elif query_type is Query_type.US_GUN_VIOLENCE:
            self.get_USGunViolence_list()
        else:
            raise 'Query_list::create_query_set(): There is no query_type provided.'


    def get_USFlightDelay_list(self):
        '''
    ...
    def get_USGunViolence_list(self):
